# Rei da Informática VIP

Site estático otimizado para publicar no GitHub Pages, Hostinger, Netlify, Vercel ou qualquer hospedagem simples.

## O que foi melhorado

- HTML refeito com estrutura válida e metatags de SEO.
- Páginas de drivers com conteúdo mais completo, FAQ e Schema.org.
- Links de download apontando para páginas oficiais dos fabricantes.
- Busca e filtro de drivers por modelo/marca.
- Página de suporte com ofertas comerciais e CTA para WhatsApp.
- Política de privacidade, `robots.txt`, `sitemap.xml` e `ads.txt`.
- Espaços de publicidade marcados como "Publicidade".

## Configuração obrigatória para monetização

1. Publique todos os arquivos da pasta `blog-ti` no domínio `reidainformatica.vip`.
2. Confirme o domínio no Google Search Console e envie `https://reidainformatica.vip/sitemap.xml`.
3. No AdSense, crie blocos de anúncio responsivos.
4. Abra `script.js` e preencha os IDs em `AD_SLOTS`.
5. Mantenha o arquivo `ads.txt` na raiz do domínio.
6. Configure Analytics ou outro painel para medir páginas mais acessadas e conversões no WhatsApp.

## Observação comercial

Receita de anúncios depende de tráfego, nicho, RPM, aprovação do AdSense e otimização contínua. A parte de vendas depende da oferta, velocidade de resposta no WhatsApp e taxa de fechamento.

